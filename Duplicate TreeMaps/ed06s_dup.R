...
# ----------------------------------------------------------------------------------
# Read-in only ED 2006 Data
setwd("~/GitHub/ICD-9_DV")
ed06 <- read.csv("ed06s.csv")
...
ed06 <- ed06[,myvars] #smaller dataset with columns of interest to reduce processing time

# After subset, we apply the survey design weights 
library(survey)
wt<-svydesign(data=ed06, # dataset from ed.list by appropriate year
...
sum(svytable(~DIAG1,wt)) #estimated 115,322,815 visits to ED in 2006

newtab <- svytable(~DIAG1,wt) # only considering DIAG1
ed06.df <- as.data.frame(newtab, stringsAsFactors = F)
names(ed06.df)
head(ed06.df)
...
# ----------------------------------------------------------------------------------
# ADDING 3 digit code/classification to ed06.df (based on diag.c)
ed06.df$Code <- "-"
for (k in paste0("^",diag.c[,2])){ #codes
  gindex <- grep(k,ed06.df[,1]) #matches code with DIAG1 column
  ed06.df$Code[gindex] <- diag.c$Code[grep(k,diag.c[,2])]
  ed06.df$Code.n <- as.numeric(ed06.df$Code) #some values are coered to NA
  ed06.df$Code.n[which(is.na(ed06.df$Code.n))] <- 1000 #treat all NA's wih value 1000
}

# ----------------------------------------------------------------------------------
# Use of dplyr 'join()' for classification of diagnoses
# ----------------------------------------------------------------------------------
head(join(ed06.df,diag.c,by="Code")) #join ed06.df with names from diag.c from plyr package

ed06.df <- join(ed06.df,diag.c,by="Code")
head(ed06.df)
str(ed06.df)
# ----------------------------------------------------------------------------------
...
dt.ed06 <- data.table(ed06.df)
dt.ed06[, Code.n2 := Code.n] #create a range of zero for overlap
dt.ed06
setcolorder(dt.ed06, c("DIAG1","Freq","Code","Classification","Code.n","Code.n2"))
names(dt.ed06)
setkey(dt.ed06)

# Apply foverlaps to original dt.ed06
dt.ed06 <- foverlaps(dt.ed06, dt.g, by.y = c("Start.n","End.n"), by.x = c("Code.n","Code.n2"), type = "within", mult="all", nomatch = 0L) #removes V and Blanks, only1931 rows
# ----------------------------------------------------------------------------------
...
dt.ed06 <- foverlaps(dt.ed06, dt.sg, by.y = c("Start.n","End.n"), by.x = c("Code.n","Code.n2"), type = "within", mult="all", nomatch = 0L) #removes V and Blanks, only1931 rows

dt.ed06 <- dt.ed06[order(Group.n)] #order the data.table by Group number (Group.n)
head(dt.ed06,10)
# we want to keep Subgroup, Range, Group, Group.n, DIAG1, Freq, Code, Classification
names(dt.ed06)
dt.ed06[,which(!grepl("Subgroup|Range|^Group$|Group.n|DIAG1|Freq|^Code$|Classification",names(dt.ed06))):=NULL]
names(dt.ed06)
setcolorder(dt.ed06,c("Group","Group.n","Subgroup","Range","Classification","Code","DIAG1","Freq"))
head(dt.ed06) #check order
str(dt.ed06) #check column classes

# Convert back to data.frame
df.ed06 <- as.data.frame(dt.ed06) #getAnywhere("as.data.frame.data.table")
str(df.ed06)
...
ICD9.code <- as.Node(df.ed06,pathName="pathString")
ICD9.code
print(ICD9.code,"Freq")

# ----------------------------------------------------------------------------------
# BY NAMES
df.ed06$pathString1 <- paste("2006",
                             df.ed06$Group,
                             df.ed06$Subgroup,
                             df.ed06$Classification,
                             sep = "/")
ICD9.name <- as.Node(df.ed06,pathName="pathString1")
ICD9.name
...
png(filename="ICD-9_tree_2006.png",width=1300, height=850,unit="px")
tree.n <- treemap(df.ed06,
                  ...
                  title="NHAMCS Emergency Department Visits in 2006",
