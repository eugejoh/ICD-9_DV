...
# ----------------------------------------------------------------------------------
# Read-in only ED 2008 Data
setwd("~/GitHub/ICD-9_DV")
ed08 <- read.csv("ed08s.csv")
...
ed08 <- ed08[,myvars] #smaller dataset with columns of interest to reduce processing time

# After subset, we apply the survey design weights 
library(survey)
wt<-svydesign(data=ed08, # dataset from ed.list by appropriate year
...
sum(svytable(~DIAG1,wt)) #estimated 115,322,815 visits to ED in 2008

newtab <- svytable(~DIAG1,wt) # only considering DIAG1
ed08.df <- as.data.frame(newtab, stringsAsFactors = F)
names(ed08.df)
head(ed08.df)
...
# ----------------------------------------------------------------------------------
# ADDING 3 digit code/classification to ed08.df (based on diag.c)
ed08.df$Code <- "-"
for (k in paste0("^",diag.c[,2])){ #codes
  gindex <- grep(k,ed08.df[,1]) #matches code with DIAG1 column
  ed08.df$Code[gindex] <- diag.c$Code[grep(k,diag.c[,2])]
  ed08.df$Code.n <- as.numeric(ed08.df$Code) #some values are coered to NA
  ed08.df$Code.n[which(is.na(ed08.df$Code.n))] <- 1000 #treat all NA's wih value 1000
}

# ----------------------------------------------------------------------------------
# Use of dplyr 'join()' for classification of diagnoses
# ----------------------------------------------------------------------------------
head(join(ed08.df,diag.c,by="Code")) #join ed08.df with names from diag.c from plyr package

ed08.df <- join(ed08.df,diag.c,by="Code")
head(ed08.df)
str(ed08.df)
# ----------------------------------------------------------------------------------
...
dt.ed08 <- data.table(ed08.df)
dt.ed08[, Code.n2 := Code.n] #create a range of zero for overlap
dt.ed08
setcolorder(dt.ed08, c("DIAG1","Freq","Code","Classification","Code.n","Code.n2"))
names(dt.ed08)
setkey(dt.ed08)

# Apply foverlaps to original dt.ed08
dt.ed08 <- foverlaps(dt.ed08, dt.g, by.y = c("Start.n","End.n"), by.x = c("Code.n","Code.n2"), type = "within", mult="all", nomatch = 0L) #removes V and Blanks, only1931 rows
# ----------------------------------------------------------------------------------
...
dt.ed08 <- foverlaps(dt.ed08, dt.sg, by.y = c("Start.n","End.n"), by.x = c("Code.n","Code.n2"), type = "within", mult="all", nomatch = 0L) #removes V and Blanks, only1931 rows

dt.ed08 <- dt.ed08[order(Group.n)] #order the data.table by Group number (Group.n)
head(dt.ed08,10)
# we want to keep Subgroup, Range, Group, Group.n, DIAG1, Freq, Code, Classification
names(dt.ed08)
dt.ed08[,which(!grepl("Subgroup|Range|^Group$|Group.n|DIAG1|Freq|^Code$|Classification",names(dt.ed08))):=NULL]
names(dt.ed08)
setcolorder(dt.ed08,c("Group","Group.n","Subgroup","Range","Classification","Code","DIAG1","Freq"))
head(dt.ed08) #check order
str(dt.ed08) #check column classes

# Convert back to data.frame
df.ed08 <- as.data.frame(dt.ed08) #getAnywhere("as.data.frame.data.table")
str(df.ed08)
...
ICD9.code <- as.Node(df.ed08,pathName="pathString")
ICD9.code
print(ICD9.code,"Freq")

# ----------------------------------------------------------------------------------
# BY NAMES
df.ed08$pathString1 <- paste("2008",
                             df.ed08$Group,
                             df.ed08$Subgroup,
                             df.ed08$Classification,
                             sep = "/")
ICD9.name <- as.Node(df.ed08,pathName="pathString1")
ICD9.name
...
png(filename="ICD-9_tree_2008.png",width=1300, height=850,unit="px")
tree.n <- treemap(df.ed08,
                  ...
                  title="NHAMCS Emergency Department Visits in 2008",
